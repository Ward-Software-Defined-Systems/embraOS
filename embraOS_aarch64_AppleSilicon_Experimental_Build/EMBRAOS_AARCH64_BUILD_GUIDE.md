...
### 2. Place files & build the image

Run everything from the embraOS repo root (cloned per the main
[README](../README.md) — `~/projects/embraOS`, with WardSONDB a sibling at
`~/projects/WardSONDB`):

```bash
cd ~/projects/embraOS

# WardSONDB is a required sibling repo (build-image-aarch64.sh builds + copies it)
[ -d ../WardSONDB ] || git clone \
  https://github.com/Ward-Software-Defined-Systems/wardsondb.git ../WardSONDB

...
Phase 1 includes **87 registered tool descriptors** — 81 canonical tools plus 6 backward-compat aliases (`memory_search`, `search_memory`, `file_rename`, `rmdir`, plus their kin). These are internal tools invoked by the intelligence during conversation — not user-facing commands. Since Sprint 3's NATIVE-TOOLS-01 migration (2026-04-22), every tool is declared to the Anthropic Messages API via the native `tools` array on each request; the Brain emits structured `tool_use` content blocks with typed JSON arguments, and embra-brain returns correlated `tool_result` blocks. No more text-tag parsing. Two post-merge passes have extended the catalog: 2026-04-23 added `knowledge_sweep_orphans` (graph hygiene) and `turn_trace` (mid-turn introspection); 2026-04-24 added `task_delete` and `plan_delete` to give the intelligence a cleanup path for throwaway plans and completed tasks. The module system (Phase 3) will introduce pluggable MCP server modules for extensibility.
...
| **Phase 1 — Sprint 3** | WardSONDB pluggable storage engine (`--storage-engine <fjall\|rocksdb>`), EXPR-01 expression panel, NATIVE-TOOLS-01 Anthropic native tool-use migration, tool-coverage expansion, post-merge and post-verification fix passes (87 tools, 135 tests) | 🚧 In Progress |
...
| `embra-brain` | Headless AI runtime — Brain, 87 tools, sessions, Learning Mode, proactive engine, knowledge graph | Complete |
...
> **🚧 In Progress** on `phase1-sprint3`. Sprint 3 PR #1 (through NATIVE-TOOLS-01 and the post-merge tool-fix pass) merged to `main` 2026-04-22; the 2026-04-24 post-verification pass is committed locally, verified end-to-end in QEMU, pending push.

Workspace grows 7 → 9 crates; registry 75 → 87 tools; 80 → 135 unit tests. The sprint delivers a pluggable WardSONDB storage engine, a persistent expression panel, and a full migration from legacy text-tag tool dispatch to Anthropic's native tool-use API.
...
- **Post-verification pass (2026-04-24)** — operational-prompt arch-strip: the ~650-token boot-chain / service-list / disk-layout block was removed from the operational prompt so the intelligence rediscovers its OS via tools + operator conversation rather than a hand-maintained prose block that goes stale each sprint. New `plan_delete` + `task_delete` tools (cascade-off default). `git_branch` delete pre-checks `git merge-base --is-ancestor` against a configurable base (closes a silent contract violation where `git branch -d` accepted any pushed branch as merged). Unified `git_*` path handling via new `resolve_workspace_path` helper. `turn_trace` off-by-N fix, migration v8 cleanup of stale trace docs, and sort-shape fix for WardSONDB's `[{field: "asc"}]` convention.

**Status:** Sprint 3 PR #1 merged to `main` 2026-04-22 (NATIVE-TOOLS-01 + post-merge tool-fix pass). 2026-04-24 post-verification pass committed on `phase1-sprint3`, 14 commits ahead of `origin/phase1-sprint3` pending push. `cargo clippy --workspace --all-targets` clean. QEMU E2E smoke pass #2 verified 2026-04-23; post-verification `turn_trace` retest confirmed 2026-04-24.

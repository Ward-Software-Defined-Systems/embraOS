...
Phase 1 includes **85 registered tool descriptors** — 79 canonical tools plus 6 backward-compat aliases (`memory_search`, `search_memory`, `file_rename`, `rmdir`, plus their kin). These are internal tools invoked by the intelligence during conversation — not user-facing commands. Since Sprint 3's NATIVE-TOOLS-01 migration (2026-04-22), every tool is declared to the Anthropic Messages API via the native `tools` array on each request; the Brain emits structured `tool_use` content blocks with typed JSON arguments, and embra-brain returns correlated `tool_result` blocks. No more text-tag parsing. The post-merge tool-fix pass (2026-04-23) added two more — `knowledge_sweep_orphans` for graph hygiene and `turn_trace` for the intelligence's mid-turn introspection. The module system (Phase 3) will introduce pluggable MCP server modules for extensibility.
...
| `embra-brain` | Headless AI runtime — Brain, 85 tools, sessions, Learning Mode, proactive engine, knowledge graph | Complete |
...
**Status:** Sprint 3 PR #1 ("Phase1 sprint3 - Tools Updates") merged to `main` 2026-04-22 (`main` and `origin/phase1-sprint3` at `fcb8035`). The 18 NATIVE-TOOLS-01 commits `ba88569` → `a7e19b6` shipped as part of that PR. A follow-up tool-fix pass on 2026-04-23 (5 commits + 1 doc-sync, closes 9 `Embra_Debug` issues — see the post-merge notes below) brought the registry from 83 → 85 descriptors and the test count from 108 → 130 workspace-wide. **QEMU E2E smoke pass #2 verified end-to-end on 2026-04-23**, covering the post-hotfix native-tool-use surface plus the 5-commit tool-fix pass. All 9 closed `Embra_Debug` issues confirmed by operator. Branch is 6 commits ahead of `origin/phase1-sprint3` pending push. `cargo clippy --workspace --all-targets` clean (0 errors; warnings all pre-existing). Grepping the repo for the legacy tag prefix outside intentional fallback-comment markers and regression-guard assertions returns only historical changelog entries. FIX-01 residual is structurally closed: the parser that scanned text for legacy tag syntax no longer exists.

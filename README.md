...
**Current Status:** Phase 1 — Core OS (Sprint 5 OPENAI-COMPAT-PROVIDER-01 + REASONING-STREAM-01 follow-up ready to merge — `phase1-sprint5` branch; Sprint 4 merged to `main` 2026-04-25 at tag `v0.4.0-phase1`) | Phase 0 — Stable
...
| **Phase 1 — Sprint 5** | OPENAI-COMPAT-PROVIDER-01 — Ollama + LM Studio as additional `LlmProvider` implementations via single `OpenAICompatProvider` with `OpenAiCompatPreset` discriminator, schema v11 with `openai_compat` config, STATE bearer plumbing (mode 0600 retroactively applied to api_key files), 4-way wizard with Endpoint → Bearer → Probe-and-Select sub-flow, defensive multi-key reasoning parser (`delta.reasoning` cookbook-primary first, `delta.reasoning_content` LM Studio newer-default fallback), `Block::ProviderOpaque{kind:"reasoning"}` CoT round-trip per OpenAI cookbook preserve/drop rules, `/provider --setup <ollama\|lm_studio>` runtime reconfigure (Endpoint → Bearer choice → Bearer token? → Model selection with bearer env hot-reload), model-aware `reasoning_effort` gating (gpt-oss / o-series / DeepSeek-R / `-thinking` variants), parameterless tool-schema `properties:{}` stamping for LM Studio zod validator, `tool_choice:"auto"` string form for OpenAI canonical, `docs/RECOMMENDED-MODELS.md` operator-facing starter doc. Plus REASONING-STREAM-01 follow-up (added 2026-05-06, operator-verified live across all 4 providers): live reasoning to the expression panel via new `StreamEvent::ReasoningDelta` + proto oneof variant 8, default-on with opt-out via `SystemConfig.show_reasoning` + `/show-reasoning <on\|off>`, panel switches source per state (italic dark-gray reasoning during a turn, solid gray operator-set expression when idle, persists past `ResponseDone` until next user submit), 64 KiB UTF-8-safe transient buffer, load-bearing privacy contract keeps reasoning out of `full_response` / session history / IR Text round-trip — Anthropic `display: "summarized"` (was `"omitted"`), Gemini `includeThoughts: true`, OpenAI-compat already-on (90 tools, embra-brain 399 tests / embra-console 16 tests / 421 workspace, workspace v0.5.0-phase1) | ✅ **Complete** |

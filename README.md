...
### Phase 1 Sprint 4 Scope

> **🔨 In Progress.** GEMINI-PROVIDER-01 introduces a pluggable LLM provider abstraction so embraOS can swap the Anthropic backend for Google's Gemini 3.1 Pro without touching the loop driver, prompts, sessions, or tools. Sprint 4 ships on `phase1-sprint4`. QEMU E2E smoke is green against the live stack (both providers, `/provider` switch, `/provider --setup` runtime key add); pre-push hold-back continues per operator close-out cadence.

**Pluggable LLM provider architecture** — the Brain's outbound LLM surface is now fronted by an `LlmProvider` trait. The loop driver in `grpc_service.rs::handle_request` consumes only a neutral intermediate representation (IR) — `Block::{Text, ToolCall, ToolResult, ProviderOpaque}` and `TurnOutcome::{EndTurn, ToolUse, MaxTokens, Pause, EarlyStop}`. Each provider owns its own wire types, request body construction, streaming parser, and tool schema translator, then translates IR ↔ wire internally. Provider crate layout under `crates/embra-brain/src/provider/`: `anthropic/{wire.rs, streaming.rs, conv.rs, tool_schema.rs, mod.rs}` for Claude Opus 4.7 (the existing Anthropic integration, refactored verbatim with no behavior change), `gemini/{wire.rs, streaming.rs, conv.rs, tool_schema.rs, cache.rs, mod.rs}` for the new Gemini backend.

**Anthropic refactor — zero behavior change.** The pre-Sprint-4 Anthropic Brain moved into `provider::anthropic` module-by-module. `MessageBlock`, `AnthropicWireMessage` (renamed from the old `ApiMessage` to avoid IR clash), `AssistantResponse`, `StopReason`, and the SSE parser all moved unchanged. The IR ↔ wire converter folds wire `Thinking` immediately followed by `ToolUse` into a single `Block::ToolCall { provider_opaque: <thinking_json> }`; standalone Thinking becomes `Block::ProviderOpaque`; round-trip preserves order in both directions across 6 unit tests. All 132 pre-existing brain tests pass after the move; cache breakpoints (system prompt, tools last entry, penultimate-message text block), extended thinking, and `tool_choice: "auto"` invariants are preserved bit-for-bit.

**Gemini 3.1 Pro backend — full tool-use loop with explicit Context Caching.** New `GeminiProvider` POSTs `:streamGenerateContent?alt=sse` with `thinkingLevel=high` (the Gemini 3.1 Pro default), `maxOutputTokens=64000`, no temperature (Gemini docs explicitly recommend leaving the default 1.0). Hand-rolled SSE parser handles delta-style chunks: text parts extend the running text accumulator if their `thought` flag matches, function_call parts always push new positions, signature-only chunks attach to the most recent non-text part (the docs-documented "signature in its own chunk" pattern). `thought:true` text parts (chain-of-thought summaries) are accumulated for round-trip but filtered from user-visible deltas. `finishReason → TurnOutcome` mapping handles `STOP+ToolCall→ToolUse`, `STOP→EndTurn`, `MAX_TOKENS`, `SAFETY`, `RECITATION`, `MALFORMED_FUNCTION_CALL`. Exponential backoff retry on 429/5xx (1/2/4/8/16/32/60s) honoring `Retry-After`; mid-stream errors are not retried.

**Gemini tool schema translator — universal coverage over all 90 tools.** Pipeline: extract `definitions`/`$defs` → inline `$ref` references → collapse single-element `allOf` (schemars's description-with-`$ref` idiom) → collapse `oneOf`-of-literal-enum (schemars's unit-variant-with-doc idiom) → reject multi-element `oneOf`/`allOf` (Gemini accepts only `anyOf` among the union keywords) → uppercase `type` values at every level (the spec missed this — Gemini wants `STRING` not `string`) → strip schema-meta keys (`$schema`, `$id`, `additionalProperties`, etc.) via deny-list. Universal regression test `all_registry_tools_translate_cleanly` exercises every registered descriptor; the parallel Anthropic guard `every_tool_schema_is_plain_object_no_top_level_combinators` is preserved.

**Context cache lifecycle (simplified per locked decision)** — `provider/gemini/cache.rs` owns one singleton handle in WardSONDB at `provider.gemini_cache:current`. `ensure_cache(system, tools)` reads the stored handle, validates session/fingerprint/TTL, and either returns the existing handle (`cache:hit`), deletes-and-recreates (`cache:miss reason=session_changed|stale|expired`), or creates fresh (`cache:create`). 4xx from `cachedContents` create returns `Ok(None)` and the caller falls back to per-request `systemInstruction` + `tools` — the Gemini docs list the Gemini 3 Pro Preview as caching-eligible but don't explicitly call out the `-3.1-` ID, so this fallback path is the safety net. Boot self-heal HEAD-probes the stored handle and clears the local doc on 404. Fingerprint = sha256 over `(system_prompt || \x00 || canonical_tools_json)`, truncated to 16 hex chars. The simplified scope drops mid-turn TTL refresh, the 4096-token defensive threshold, and the spec's 5-event telemetry taxonomy (collapsed to 4: `create`/`hit`/`miss`/`delete`).

**Wizard provider step.** First-run wizard reordered: Name → **Provider** → API key → Timezone → Confirm. The Provider step is a Selector with `Anthropic Claude Opus 4.7` (default) and `Google Gemini 3.1 Pro` options. The wizard drives the providers' own `LlmProvider::validate_key` impls — single source of truth for the model-listing probe. Per-provider error messages (Forbidden under Gemini flags missing-billing explicitly, since Gemini 3.1 Pro Preview requires a billed Google account). `ANTHROPIC_API_KEY` and `GEMINI_API_KEY` env vars are honored when the matching provider is selected; the other-provider's env-var surfaces as a `SystemMessage::Info` ("ignored") so the operator understands why the value isn't being used.

**`/provider [status|<kind>|--setup]` slash command + cross-provider session block.**

- `/provider` or `/provider status` — shows active provider, model, and session.
- `/provider anthropic` / `/provider gemini` — switches provider for future turns. Idle → swap immediately; mid-tool-loop → queue via `pending_provider` and drain after the loop completes (before next user message). Swap updates `config.system.api_provider`, the active session's `meta.provider` + `meta.model`, the legacy `api_key` mirror (so existing read paths see the right value), and `/embra/state/api_provider`.
- `/provider --setup [<kind>]` — multi-turn key add. Auto-targets the provider whose key is missing (or use `--setup gemini` to replace an existing key). Sends a `SetupPrompt::Text` asking for the key on the next message; `handle_request` intercepts that message, validates via the same probe path the wizard uses, and persists into `cfg.<target>_api_key` + `/embra/state/api_key_<target>`. On invalid: the flag stays set so the operator can retype without re-running `--setup`.
- **Cross-provider session attach is hard-blocked.** Reopening an Anthropic-recorded session under an active Gemini configuration (or vice versa) refuses with a `SystemMessage::Error` directing to `/provider <other>` or `/new`. The pre-v9 default for `meta.provider` is `"anthropic"` so legacy sessions resume cleanly under Anthropic.

**Per-provider API keys (schema v10).** `SystemConfig` gains `anthropic_api_key` + `gemini_api_key` Option<String> fields alongside the legacy `api_key` (which now mirrors the active provider's key). `SystemConfig::key_for(kind)` resolves the right key — per-provider field first, falls back to legacy `api_key` when the active provider matches, returns `None` when nothing is recorded for the inactive side. Migration v10 backfills the per-provider field from the existing `api_key` + `api_provider` pair on first boot. STATE partition gains `/embra/state/api_key_<provider>` files; supervisor prefers the per-provider STATE file with fallback to the legacy `api_key` and then the matching env var. The `/provider` precheck switches from a key-prefix heuristic to `cfg.key_for(target).is_none()` — actually checks if a key is recorded.

**Schema migration v9 (pluggable provider scaffolding).** `CURRENT_SCHEMA_VERSION` 8 → 9. Idempotent: creates `provider.gemini_cache` collection; stamps `config.system.api_provider="anthropic"` if missing; iterates every `sessions.<name>.meta` doc and stamps `meta.provider="anthropic"` + `meta.model="opus-4.7"` defaults on docs missing them. Schema migration v10 (per-provider keys, described above) follows immediately.

**Operator escape hatch — `EMBRA_GEMINI_MODEL`.** If Gemini 3.1 Pro is observed ignoring custom tools in favor of bash invocations (a documented failure mode for some Gemini 3 model variants), the operator can set `EMBRA_GEMINI_MODEL=gemini-3.1-pro-preview-customtools` before brain restart. Also persistable as `config.system.gemini_model`. Per-turn telemetry under `target="gemini::diag"` logs `text_chars`, `tool_calls`, `iters` so journalctl reveals when the customtools swap is needed (text > 0 + tool_calls == 0 across iterations).

**Status line.** `embra-console`'s status bar reads from `AppState.provider_model: String`, populated from a `Brain: <model>` token the brain now embeds in the existing `ModeTransition` message format. The previously hardcoded `opus-4.7` strings are gone; the bar reflects the active provider after wizard, `/provider` switch, or session attach.

**Test + lint state (2026-04-24):** `cargo test --workspace` 207 passed (was 142 at Sprint 3 close); `cargo build --workspace` clean; `cargo clippy --workspace --no-deps` 0 errors. 90 ToolDescriptors unchanged. The pre-refactor brain test count (132) was preserved verbatim across the Stage 2 refactor (no test loss); +75 new tests across the Sprint 4 stages cover the provider trait surface, the IR ↔ wire converters, the Gemini schema translator + SSE parser + provider impl, the cache lifecycle, the wizard step inference, the v9 + v10 migration patches, and the per-provider key lookup matrix.

**Verified in QEMU smoke (2026-04-24):** SSE event framing (standard `data: <json>\n\n` per the SSE spec; the parser handles the Gemini chunk shape correctly), `/provider` switch with the post-smoke key-resolution fix in place, `/provider --setup` runtime key add for the alternate provider, cross-provider session attach hard-block. Gemini 3.1 Pro caching eligibility (the docs list Gemini 3 Pro Preview but not the `-3.1-` ID specifically — Stage 6 falls back to per-request system+tools if create returns 4xx) and the customtools fallback (one-line config swap, no rebuild needed) remain available as runtime escape hatches.

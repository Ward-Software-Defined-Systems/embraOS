//! Styled-text parsers (markdown headers, bold, inline code, JSON).
//!
//! Returns neutral `StyledSegment`s with `TextStyle` colors/modifiers.
//! UI front-ends adapt these to their own toolkit at the render boundary.

use crate::style::{Color, Modifier, TextStyle};
...
pub fn parse_styled_line(line: &str, base_style: TextStyle) -> StyledLine {
        ...
        || trimmed.starts_with("# ")
    {
        return vec![StyledSegment::new(
            line.to_string(),
            TextStyle::new().fg(Color::Cyan).add_modifier(Modifier::BOLD),
        )];
    }
...
pub fn parse_json_line(line: &str) -> StyledLine {
    ...
    let base_style = TextStyle::new().fg(Color::White);
    ...
    while i < len {
        ...
        if ch == '"' {
            ...
            segments.push(StyledSegment::new(s, TextStyle::new().fg(color)));
        }
        // Numbers
        else if (ch.is_ascii_digit() || ch == '-')
            && (current.trim().is_empty() || current.ends_with(": ") || current.ends_with(','))
        {
            if !current.is_empty() {
                segments.push(StyledSegment::new(current.clone(), base_style));
                current.clear();
            ...
            segments.push(StyledSegment::new(num, TextStyle::new().fg(Color::Yellow)));
        ...
        // Boolean/null keywords — check from chars array, not byte-indexed line slice
            ...
            && matches!(
                ...
                ('t', Some('r'), Some('u'), Some('e')) | ('n', Some('u'), Some('l'), Some('l'))
            )
        {
            if !current.is_empty() {
                segments.push(StyledSegment::new(current.clone(), base_style));
            ...
            segments.push(StyledSegment::new(word, TextStyle::new().fg(Color::Yellow)));
            i += 4;
            continue;
        } else if i + 5 <= len
            && (chars[i], chars.get(i + 1), chars.get(i + 2), chars.get(i + 3), chars.get(i + 4))

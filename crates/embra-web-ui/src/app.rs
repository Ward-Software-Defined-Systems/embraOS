...
const SPECS: &[Spec] = &[
    ...
    Spec { cmd: "/git-token", title: "Self-hosted git token",
        note: "Per-host token (GitLab/Gitea). Blank = list hosts; token 'remove' deletes.", join: " ", guided: false,
        fields: &[t("Host", "gitlab.example.com (blank = list)", false),
            Field { label: "Token", ph: "glpat-… or 'remove'",
                req: false, secret: true, choices: &[] }] },
    ...
    Spec { cmd: "/image-provider", title: "Image generation backend",
        note: "image_generate's backend. Blank = show provider, model and key status; 'gemini' selects Gemini image models; 'clear' resets to the defaults.",
        join: " ", guided: false,
        fields: &[sel("Action", &["(show status)", "gemini", "clear"], false)] },
    Spec { cmd: "/image-provider model", title: "Image model",
        note: "gemini-3-pro-image is the default (4K-capable); the Flash models are faster/cheaper; Flash-Lite renders 1K only.",
        join: " ", guided: false,
        fields: &[sel("Model", &["gemini-3-pro-image", "gemini-3.1-flash-image", "gemini-3.1-flash-lite-image", "gemini-2.5-flash-image"], true)] },
    Spec { cmd: "/image-provider key", title: "Image-generation API key",
        note: "Dedicated Gemini key for image_generate, stored to STATE (0600), never echoed. Blank = status; 'remove' deletes it. Without it the Gemini LLM key is used when configured.",
        join: " ", guided: false,
        fields: &[Field { label: "API key", ph: "AIza… (blank = status, 'remove' = delete)",
            req: false, secret: true, choices: &[] }] },
    Spec { cmd: "/effort", title: "Effort",
        note: "Set the active provider's effort (next message), or show current. Anthropic default max; Gemini high; local presets send the value verbatim (Qwen3.8: low/medium/xhigh).",
        join: " ", guided: false,
        fields: &[sel("Effort",
            &["(show current)", "low", "medium", "high", "xhigh", "max", "reset"], false)] },
    ...
    Spec { cmd: "/guardian show", title: "Show Guardian tool source",
        ...
        fields: &[t("Tool name", "web_search", true)] },
    Spec { cmd: "/guardian approve", title: "Approve a proposed tool",
        note: "Build + enable a tool the intelligence proposed (passed the replicant check).",
        join: " ", guided: false,
        fields: &[t("Tool name", "example_tool", true)] },
    Spec { cmd: "/guardian reject", title: "Reject a proposed tool",
        note: "Discard a pending proposal.", join: " ", guided: false,
        fields: &[t("Tool name", "example_tool", true)] },

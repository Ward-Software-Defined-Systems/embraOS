...
| `/provider --setup <ollama\|lm_studio>` | Reconfigure endpoint URL, bearer token, and selected model for an OpenAI-compat preset — 4-step flow (Endpoint → Bearer choice → Bearer token? → Model selection). Pre-fills current values; cancel anytime with any other slash command. Bearer hot-reloads via `EMBRA_<PRESET>_BEARER` env var (no brain restart). The endpoint may be any OpenAI-compatible server, not just a local one — enter the base URL without `/v1` (`https://openrouter.ai/api`, `https://api.together.xyz`, `https://api.fireworks.ai/inference`, `http://vllm-host:8000`) and the key at the Bearer step; https endpoints keep their implicit 443, only bare hosts and `http://` get the preset's default port |
...
| `/effort` | Show the active provider's effort level, its default, and the value actually sent on the wire |
| `/effort <low\|medium\|high\|xhigh\|max>` | Set the effort level for the **active** provider; takes effect on the next user message. Stored per provider (`anthropic_effort`, `gemini_effort`, `openai_compat.ollama_effort` / `lm_studio_effort`), so a `/provider` switch never changes a level you tuned. Anthropic: sent as `output_config.effort` (default `max`; lower effort trades depth for latency/cost — relevant on Fable 5, where `max`-effort turns can run many minutes). Gemini: mapped to `thinkingLevel` (default `high`; `xhigh`/`max` send `high`, Gemini 3.1 Pro's ceiling). Ollama / LM Studio: sent **verbatim** as `reasoning_effort` in place of the per-model auto-map — the accepted set belongs to the model (Qwen3.8: `low`/`medium`/`xhigh`, default `xhigh`; gpt-oss: `low`/`medium`/`high`; DeepSeek-V4-Pro: `max`) and the server validates; `/effort` warns when the level is outside the model's documented set. Unset = today's behaviour (Qwen models get no field, so LM Studio's per-model default applies). The dev-only `EMBRA_ANTHROPIC_EFFORT` / `EMBRA_GEMINI_EFFORT` / `EMBRA_OLLAMA_EFFORT` / `EMBRA_LM_STUDIO_EFFORT` env vars take precedence |
| `/effort reset` | Clear the active provider's stored level (back to its default) |

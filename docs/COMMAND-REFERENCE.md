...
| `/model <opus-5\|fable-5>` | Switch the Anthropic model (Anthropic provider only; default opus-5). Persists to `SystemConfig.anthropic_model`; takes effect on the next user message — the provider is rebuilt per turn. The `EMBRA_ANTHROPIC_MODEL` env var takes precedence over the persisted value. Switching models is a one-time prompt-cache reset (caches are model-scoped). Legacy persisted `opus-4.7`/`opus-4.8` values keep working but are no longer selectable |

...
The shorter inventory of KG tools (as part of the broader 116-tool catalog) lives in [TOOL-REFERENCE.md](TOOL-REFERENCE.md). The architectural placement (the 7-layer model's *Memory & Knowledge* row) is in [SYSTEM-DESIGN.md](SYSTEM-DESIGN.md).
...
**Operator surface:** `/embeddings` reports the model, where it was found, index size and how many nodes are embedded, per collection; `/embeddings on|off`; `/embeddings backfill [--force]` embeds what needs it (~55 ms/node measured, ~1 minute per thousand nodes), reports progress, and is resumable because the work set is re-derived from disk each run. Backfill is never automatic.
...
- Every committed pack is parsed + validated by `committed_seed_packs_validate` at test time — an invalid pack fails `cargo test` before it can ship in an image.
...
4. **Trigger auto-enrichment.** Send a substantial user message (≥15 chars, not on the chatty-filler list) that mentions `cert refresh`. Auto-enrichment fires before the model call — it doesn't go through a `knowledge_*` tool, so the trigger is just the operator typing. Watch the tracing output (or `journalctl` if you've wired it through) for the info-level `auto-enrichment` log line:
   ...
   INFO auto-enrichment session=<name> tag_count=3 candidates_total=42 candidates_direct=9
        candidates_session=21 candidates_graph=12 result_count=1 top_score=0.45
   ```
...
- [TOOL-REFERENCE.md](TOOL-REFERENCE.md) — catalog of all 116 tools the intelligence draws from (the **Knowledge Graph** table covers these twelve).

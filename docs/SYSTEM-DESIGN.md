...
- **Anthropic Claude** — Opus 5 (default), Opus 4.8, or Fable 5, selectable at the first-boot wizard, via `/model`, or the `EMBRA_ANTHROPIC_MODEL` env var (Opus 5 adopted as default on its 2026-07-24 release day; Opus 4.7 retired from selection — an instance persisted on that alias keeps resolving to its real API id via legacy canonicalization in the model resolver, but `/model` no longer offers it). Requests sent with `output_config.effort` (default `max`, runtime-tunable via `/effort`) and `thinking.display=summarized` (`=omitted` when `/show-reasoning off`) — the request shape is identical across supported models (Opus 5's thinking-on-by-default equals the explicit `adaptive` we already send; its effort-capped `thinking:disabled` is never sent). Summarized thinking deltas stream live to the expression panel; signed thinking blocks still round-trip via `Block::ProviderOpaque` for the model's own continuity. Native ephemeral prompt caching with breakpoints on system + penultimate user message + last tool entry. Transient API errors (408/409/429/5xx — including 529 overloaded) are retried with exponential backoff (1 s → 60 s ladder, `Retry-After` honored) before surfacing as an error; established streams are never retried mid-flight. Note: Fable 5 requires 30-day API data retention (a zero-data-retention org gets a 400 on every request).
- **Google Gemini 3.1 Pro** — requests sent with `thinkingLevel=high` by default (operator-tunable via `/effort`: `low`/`medium`/`high`; `xhigh`/`max` clamp to `high`), `includeThoughts=true` (`false` when `/show-reasoning off`), and `maxOutputTokens=64000`; 429/5xx retried with the same exponential-backoff ladder as the Anthropic backend. Thought-flagged parts stream live to the expression panel; thoughtSignatures stay on `Block::ProviderOpaque`. Native explicit context cache via a `GeminiCacheManager` singleton with `(session, fingerprint, TTL)` validation.
...
The loop driver consumes a neutral intermediate representation (`Block::{Text, ToolCall, ToolResult, ProviderOpaque}` and `TurnOutcome::{EndTurn, ToolUse, MaxTokens, Pause, EarlyStop}`); each provider owns its own wire types, streaming parser, and tool schema translator. All 116 tools work identically across all four backends. Sessions are pinned to the provider that recorded them — cross-provider session attach is hard-blocked. The brain probes the active provider's endpoint ~30 s after boot (the model check the config load never did), every 5 minutes, and right after a `/provider` switch or setup flow — reachability, model presence, key state, latency — and publishes the result to `/status`, the `system_status` tool and the `GetSystemStatus` RPC, which apid's REST `/status` proxies for the web console's `provider` pill; state changes reach the operator as notifications. Ollama and LM Studio share a single `OpenAICompatProvider` with a `ProviderKind` discriminator, and either preset reaches **any** OpenAI-compatible server — vLLM, Together, Fireworks, OpenRouter, a remote Ollama — by pointing its endpoint there via `/provider --setup <ollama|lm_studio>`: enter the base URL without `/v1` (`http://vllm-host:8000`, `https://api.together.xyz`, `https://api.fireworks.ai/inference`, `https://openrouter.ai/api`) and the API key at the Bearer step; https endpoints keep their implicit 443 (only bare hosts and `http://` get the preset's default port). The status bar keeps showing the preset's label. No further named presets are planned. Terminal `refusal` and `max_tokens` outcomes surface operator-facing error/warning frames (refusal detail parsed from the API's `stop_details`; history persists an explicit marker instead of a silent `(no response)`) — and a refusal is never silently re-served by another model: the Anthropic API's opt-in `fallbacks` parameter is deliberately unused, because embraOS binds a single model to a sealed identity. The operator can also interrupt a turn (`/stop` / Esc / the mobile ■ button — built for local-model reasoning loops that the tool-iteration cap never catches): the signal travels as an out-of-band unary `StopTurn` RPC because the Converse loop processes requests sequentially, and it works because every provider's streaming pump exits when its receiver is dropped — severing the HTTP connection is what actually stops a local server's generation. A stopped turn keeps its partial text, marked `(response interrupted by operator stop)`.

**Reasoning controls per family:**
- **Operator override (`/effort`)** — while an Ollama or LM Studio preset is active, `/effort <low|medium|high|xhigh|max>` stores a per-preset level (`openai_compat.ollama_effort` / `lm_studio_effort`) that is sent **verbatim** as `reasoning_effort` in place of the auto-map below; the accepted set belongs to the model and the server validates (`/effort` warns when the level is outside the model's documented set). Unset = the auto-map, byte-identical to before the knob existed.
- **gpt-oss / OpenAI o-series / DeepSeek-R1·R2 / `-thinking` variants** — embraOS sends OpenAI-compat `reasoning_effort: "high"` automatically (route table `reasoning_effort_for_model`).
...
- **Qwen3 family** (Qwen3, Qwen3.6, including `*-A3B` MoE) — thinking is integrated into the same model and toggled via `/think` and `/no_think` directives in user/system messages. `reasoning_effort` is omitted by default to avoid `No valid custom reasoning fields found` server warnings. **Qwen3.8** is the exception: it accepts `reasoning_effort` `low`/`medium`/`xhigh` (default `xhigh`, per its model card) — set it with `/effort`; unset, LM Studio's per-model default applies. See `RECOMMENDED-LOCAL-MODELS.md` for the vetted roster and server configuration.

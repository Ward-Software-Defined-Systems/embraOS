...
| Tool | Description |
|---|---|
| **file_read** | Read file contents or list directory entries (up to 1000). Reads files whole by default — one call returns up to the per-call ceiling (2 MiB minus a 4 KiB framing reserve, derived from the dispatcher cap so the continuation trailer always survives truncation), and a larger file ends with a trailer naming the exact resume offset. Optional `offset`/`limit` (bytes, JSON args) are for resuming from that trailer or deliberate slices; a below-ceiling `limit` that stops short of EOF earns an omit-limit tip. Unrestricted path. Handles binary files gracefully |

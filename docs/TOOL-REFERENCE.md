...
| **knowledge_query** | Context-aware retrieval — exact tag match, IDF-weighted content-token match, session context, **semantic similarity**, multi-signal ranking. Every step window is recency/rank-sorted (tags: newest 100/tag, matched in-memory over a per-call node prefetch; **content: token matching over semantic content, procedural title+description, AND entry content**, weighted by inverse document frequency with a stopword floor so a hit on `plan` outranks three hits on `the`; session edges: all 50 newest entries walked at bounded concurrency, entry targets excluded server-side). **Depth-2 graph expansion was REMOVED 2026-09-08** — measured on a production graph it cost ~96% of retrieval latency while contributing nothing to the injected top-20; `knowledge_traverse` is unaffected. In its place, a sentence-embedding model running in-process ranks candidates by meaning: the top matches above a cosine floor are admitted, and every lexically-found candidate is also scored, so similarity corrects lexical noise rather than competing with it. Scoring relevance = max(tag relevance, similarity-or-content) — where a node has a vector its similarity REPLACES the length-biased lexical score. Supports `query \| max \| categories_csv` syntax. Output shows the returned-set source breakdown (direct/session/other) plus a pre-ranking `Candidates considered` line reporting how many candidates similarity contributed. Promoted-entry/target pairs are deduplicated so the same claim doesn't fill two slots. Access-touches only the final returned top-K |
...
| **file_copy** | Copy a file or a directory tree host-side — contents never enter the conversation, so a generated image in `/embra/workspace/MEDIA/` reaches a repo intact and a backup/restore costs the call, not the bytes. `source` may be ANY readable path (file_read's read policy); `destination` is the exact final path under `/embra/workspace` — never a directory to copy INTO (refused with the full-path hint), never a tree merge. Files copy atomically through the shared temp + fsync + rename tail (`file_patch::commit_temp`), with the source's mode on a new destination; `overwrite` (files only) replaces an existing file atomically and keeps ITS mode/ownership; the report carries absolute bytes, mode, and a sha256 of both source and destination. Directories need `recursive`; the destination must not exist; entries are walked sorted, symlinks recreated verbatim (never followed — a link whose target would resolve outside the workspace at its new location is skipped and reported, the `file_symlink` rule), special files skipped, `.git` copied unless named in `exclude` (exact basenames, no globs); any failure rolls the whole tree back (rename-aside-then-remove, also on the 10-minute timeout). Budgets are hard: 512 MiB per call counted on bytes actually READ (stat sizes are advisory — a procfs file cannot fill DATA), 100 000 entries, and 256 MiB must stay free on the data partition (`statvfs` before any write). `dry_run` walks, validates and reports without creating anything — not even parent directories |
